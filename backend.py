...
df_dropped = raw_data_df.drop(columns=['X', 'Y', 'OBJECTID', 'LONG_WGS84',
                                       'LAT_WGS84'])  # the requirements suggest not analyzing the masked location data
corr_matrix_num_to_num: pd.DataFrame = df_dropped.select_dtypes(include=[np.number]).corr()
# print(corr_matrix_num_to_num)
corr_matrix_num_to_num_filtered = corr_matrix_num_to_num.where(
    (corr_matrix_num_to_num > 0.5) | (corr_matrix_num_to_num < -0.5))
...
missing_data_df = raw_data_df.isnull().sum().to_frame(
    name='missing_count')  # both sns and plt.imshow() require a dataframe
